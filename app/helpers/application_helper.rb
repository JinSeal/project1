module ApplicationHelper
    ...
    def get_watchlist_info
        ...
        @current_user.watchlists.each do |item|
            ...
            selected_info = {:id => item.id, :symbol => item.symbol, :company =>info.company_name, :latest_price => info.latest_price, :change_percent => info.change_percent, :previous_close => info.previous_close, :open => info.open, :volume => info.volume}
    ...
    def get_portfolio_summary
        ...
        if portfolios_details.present?
            ...
            arrays_by_folio_id.each{|folio_details_by_id|
                ...
                # else
                # investment = today_change = total_profit =0
                # id = portfolios_details[portfolios_summary.length-1][:id]
                # name = Portfolio.find(id)[:name]
                # end
